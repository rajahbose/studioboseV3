/**
 * convert-train-station.js
 * Converts all train-station images to AVIF format,
 * resized to 4K (3840px wide max), optimized for web.
 *
 * Usage: node convert-train-station.js
 */
...
const PROJECT_DIR = path.join(__dirname, 'images', 'projects', 'train-station');
const OUTPUT_DIR  = PROJECT_DIR; // write alongside originals

// 4K-optimized AVIF settings
const AVIF_OPTIONS = {
  quality: 80,
  effort: 5,
...
async function convertToAvif() {
  ...
  const results = [];
  const nameMap = []; // tracks { original, avifName } for projects.js reference

  for (let i = 0; i < files.length; i++) {
    const file  = files[i];
    const num   = String(i + 1).padStart(2, '0');
    const outputName = `train-station-${num}.avif`;
  ...
  console.log(nameMap.map(n => `  "images/projects/train-station/${n}"`).join(',\n'));
}
